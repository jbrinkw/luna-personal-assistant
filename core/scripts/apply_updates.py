...
def phase_1_check_queue(repo_path):
    ...
    if not queue_path.exists():
        log("No queue found, restarting bootstrap...")
        # Restart bootstrap
        bootstrap_path = repo_path / "luna.sh"
        os.execv(str(bootstrap_path), [str(bootstrap_path)])
        sys.exit(0)
...
def phase_9_cleanup_and_restart(repo_path):
    """
    Phase 9: Cleanup and Restart
    Delete this script and restart bootstrap
    """
    log("Phase 9: Cleanup and restart...")
    ...
    # Restart bootstrap
    bootstrap_path = repo_path / "luna.sh"
    log(f"Restarting system: {bootstrap_path}")
...
def main():
    ...
    try:
        ...
        # Phase 9: Cleanup and restart
        phase_9_cleanup_and_restart(repo_path)
